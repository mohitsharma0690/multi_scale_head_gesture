...
)
{
    ...
    int     n, n5, iter, status, i,
            cg_tol (double, double, cg_parameter *) ;

    cg_parameter Parm ;

/* initialize parameters for C++ coding  */
            zero = 0;
			delta2 = 0;
			eta_sq = 0;
			Qk = 0;
...
/* initialize the parameters */
	...
	//Modified by LP 08/31/06
	// Init Parm structure with default parameters
    Parm.n = dim ;
    Parm.nf = 0 ;
    Parm.ng = 0 ;
	Parm.delta	= 0.1;
	...
	Parm.PertRule = 1;
	Parm.QuadStep = 1;
	Parm.PrintLevel = 0; // We want no print out if DebugLevel is 0
	Parm.PrintFinal = 0; // hsalamin 2010.02.05
	Parm.StopRule = 1;
	Parm.AWolfe = 0;
	Parm.Step = 0;
	Parm.debug =0;
    ...
    if ( status < -1 ) goto Exit ; // Modified by LP
	if(maxit < 0)
		maxit = Parm.maxit ;
...
/*
    ...
    maxit - range [0, infty] terminate in maxit //Modified by LP 04/24/06
...
)
{
    ...
    Parm->nrestart = dim*(int)restart_fac ;
    fgets (junk, MAXLINE, ParmFile) ;
    if (strlen (junk) >= MAXLINE-1) return (-3) ;
...
//    Parm->maxit = (int)dim*maxit_fac ;
	Parm->maxit = (int)maxit_fac ;
    fgets (junk, MAXLINE, ParmFile) ;
    if (strlen (junk) >= MAXLINE-1) return (-3) ;
