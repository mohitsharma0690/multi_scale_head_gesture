...
local MyLSTM, parent = torch.class('nn.MyLSTM', 'nn.Module')
...
function MyLSTM:__init(input_dim, hidden_dim)
  parent.__init(self)
...
function MyLSTM:set_name(name)
  self.name = name
end

function MyLSTM:reset(std)
  if not std then
...
function MyLSTM:_unpack_input(input)
...
function MyLSTM:_get_sizes(input, gradOutput)
...
function MyLSTM:updateOutput(input)
  ...
  if not c0 then
    ...
    if c0:nElement() == 0 or not self.remember_states then
      -- Start with new cell state if we don't have to remember states
      c0:resize(N, H):zero()
    elseif self.remember_states then
      -- (Mohit): We are remembering states across multiple runs here?
      local prev_N, prev_T = self.cell:size(1), self.cell:size(2)
      assert(prev_N == N, 'batch sizes must be constant to remember states')
      -- Copy value for the last state i.e. state for all batches at timestep T
      -- into c0 (since we remember states)
  ...
  if not h0 then
    ...
    if h0:nElement() == 0 or not self.remember_states then
      -- Start with new hidden state if we don't remember states and this is the
      -- first time.
      h0:resize(N, H):zero()
    elseif self.remember_states then
      local prev_N, prev_T = self.output:size(1), self.output:size(2)
      assert(prev_N == N, 'batch sizes must be the same to remember states')
      -- Copy value of last hidden state into the current one.
      h0:copy(self.output[{{}, prev_T}])
    end
  end

  -- Creates a copy of bias for all the N elements in the batch.
  local bias_expand = self.bias:view(1, 4 * H):expand(N, 4 * H)
  -- Extract 1 to D rows from weight. Note weight is (D+H, 4*H)
  local Wx = self.weight[{{1, D}}]
  -- Extract D+1 to H rows from weight. These correspond to the W_i, W_f, etc of H
  local Wh = self.weight[{{D + 1, D + H}}]

  -- Allocate output and cell state both (N, T, H)
  ...
  for t = 1, T do
    -- Take the values at current timestamp(t) from the 3D tensor.
    local cur_x = x[{{}, t}]
    local next_h = h[{{}, t}]
    local next_c = c[{{}, t}]
    local cur_gates = self.gates[{{}, t}]
    -- We do this in the reverse order since weight values for each example is
    -- along the column for us.
    -- curr_gates = bias_expand + curr_x * Wx
    cur_gates:addmm(bias_expand, cur_x, Wx)
    -- curr_gates = curr_gates + prev_h * Wh
    cur_gates:addmm(prev_h, Wh)
    -- Use tanh for cell state for all other cases we use sigmoid
  ...
  -- Our output here is the hidden state and not an explicit output
  return self.output
end


function MyLSTM:backward(input, gradOutput, scale)
  ...
  for t = T, 1, -1 do
    ...
    -- The gradients calculated here are with respect to the activations function
    -- being used in each gate. This allows us to find the required dl/dw and
    -- dl/db derivatives directly.
    ...
    -- grad_next_c
    -- NOTE: As mentioned above these are just being used temporarily
    -- calculate dl/dc[t] = (dl/dh[t]) * (dh[t]/dc[t])
    -- dl/dc[t] = (dl/dh[t]) * o[t](1-tanh(c[t])^2)
    ...
    -- i and f are simple sigmoid functions that depend on dl/dc[t]
...
function MyLSTM:updateGradInput(input, gradOutput)
...
function MyLSTM:accGradParameters(input, gradOutput, scale)
  if self.recompute_backward then
