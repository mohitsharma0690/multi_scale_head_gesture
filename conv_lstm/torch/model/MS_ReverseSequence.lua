...
-- nn.MS_ReverseSequence(1):forward(tensor) would give: torch.Tensor({{6,7,8,9,10},{1,2,3,4,5}})
------------------------------------------------------------------------
local MS_ReverseSequence, parent = torch.class("nn.MS_ReverseSequence", "nn.Module")

function MS_ReverseSequence:__init(dim)
    parent.__init(self)
...
function MS_ReverseSequence:reverseOutput(input)
...
function MS_ReverseSequence:updateOutput(input)
    if (self.dim == 1) then
...
function MS_ReverseSequence:reverseGradOutput(gradOutput)
...
function MS_ReverseSequence:updateGradInput(inputTable, gradOutput)
    if (self.dim == 1) then
