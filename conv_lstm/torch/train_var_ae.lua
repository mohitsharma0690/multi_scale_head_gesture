...
local LSTMVarAutoEncoder = require 'model.LSTMVarAutoEncoder'
...
cmd:option('-model_type', 'lstm_autoencoder')
...
elseif opt.model_type == 'lstm_autoencoder' then
  LSTMVarAutoEncoder:createAutoencoder(
      torch.Tensor(torch.Tensor(opt.batch_size, 32, opt.num_features)))
  model = LSTMVarAutoEncoder.autoencoder
elseif opt.model_type == 'lstm_predictor' then
  LSTMVarAutoEncoder:createPredictor({opt.batch_size,32, opt.num_features})
  model = LSTMVarAutoEncoder.autoencoder
elseif opt.model_type == 'lstm_sequence_completor' then
  LSTMVarAutoEncoder:createSequenceCompletor({opt.batch_size, 10, opt.num_features})
  model = LSTMVarAutoEncoder.autoencoder
elseif opt.model_type == 'lstm_next_step_predictor' then
  LSTMVarAutoEncoder:createNextStepPredictor({opt.batch_size, 32, opt.num_features})
  model = LSTMVarAutoEncoder.autoencoder
end

assert(model ~= nil)
print(LSTMVarAutoEncoder.dummyContainer)
model:cuda()
...
local curr_KL_loss, KL_loss_history = 0, {}

-- This function is for the non-dense conv lstm model
local function save_grad_weights_dense(model, loss, KLLoss) 
  ...
  local dec_wt, dec_grad_wt = seq_model:get(3):get(2):parameters()
  ...
  curr_KL_loss = KLLoss
  table.insert(KL_loss_history, KLLoss)
...
local function f(w)
  ...
  local reconstruction_loss_ratio, KL_loss_ratio = 0.99, 0.01
  local loss = crit:forward(scores, y)
  loss = reconstruction_loss_ratio * loss

  -- Run the criterion and model backward to compute gradients
  local grad_scores = crit:backward(scores, y)
  -- reconstruction_loss_ratio acts as scale
  grad_scores = grad_scores:mul(reconstruction_loss_ratio)
  model:backward(x, grad_scores)

  -- KL-divergence loss
  local nElements = opt.batch_size
  local mean, logvar_sq = table.unpack(LSTMVarAutoEncoder.encoder.output)
  local var = torch.exp(logvar_sq)
  local KLLoss = 0.5 * torch.sum(torch.pow(mean, 2) + var - logvar_sq - 1)
  KLLoss = KLLoss / nElements
  KLLoss = KL_loss_ratio * KLLoss
  loss = loss + KLLoss
  local gradKLLoss = {mean/nElements, 0.5*(var - 1) / nElements}
  -- KL_Loss_ratio acts as scale
  gradKLLoss[1] = gradKLLoss[1]:mul(KL_loss_ratio)
  gradKLLoss[2] = gradKLLoss[2]:mul(KL_loss_ratio)
  LSTMVarAutoEncoder.encoder:backward(x, gradKLLoss)

  if opt_clone.debug_weights == 1 then
    save_grad_weights_dense(model, loss, KLLoss)
  end
...
for i = 1, opt.max_epochs do
  ...
  while coroutine.status(data_co) ~= 'dead' do
    ...
    if curr_batches_processed < total_train_batches then
      ...
      if (opt.print_every > 0 and
        ...
        local float_epoch = i
        local msg = 'Epoch %.2f, total epochs:%d, loss = %f, KLLoss = %f'
        local args = {msg, float_epoch, opt.max_epochs, 
          loss[1]-curr_KL_loss, curr_KL_loss }
  ...
  if (check_every > 0 and i % check_every == 0) then
            ...
            for k, v in ipairs(val_data_to_save) do
              if i == v then 
                ...
                if idx <= inp_tensors[y]:size(1) and i-counter <= xv:size(1) then
                  inp_tensors[y][{{idx},{},{}}] = xv[{{i-counter},{},{}}]:float()
    ...
    checkpoint.model = {
      ...
      cellSizes=LSTMVarAutoEncoder.cellSizes,
      encLSTMs=LSTMVarAutoEncoder.encLSTMs,
      decLSTMs=LSTMVarAutoEncoder.decLSTMs,
    }
