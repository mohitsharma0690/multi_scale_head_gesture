...
class DataLoader(object):
    ...
    def __init__(self, config, num_classes, num_features,
      sliding_window_length, num_classify=5, binary_classes_to_classify=None):
      ...
      self.binary_classify = binary_classes_to_classify
    ...
    def binary_classification(self, gest_type):
      if gest_type in self.binary_classify:
        return 1
      else:
        return 0

    def five_class_classification(self, gest_type):
      if gest_type == 0:
        return 0
    ...
    def classify_into_classes(self, trainX, valX, testX):
      ...
      if self.config[USE_ONE_VS_ALL]:
        ...
        for i in xrange(self.num_classes):
          if self.num_classify == 5:
            new_idx = self.five_class_classification(i)
          elif self.num_classify == 2:
            new_idx = self.binary_classification(i)
          elif self.num_classify == 11:
            new_idx = i
          else:
            assert(False)
    ...
    def load_data_centered_padded(self):
      ...
      __num_classes = self.num_classify if self.config[USE_ONE_VS_ALL] else self.num_classes
